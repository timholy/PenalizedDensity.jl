...
`κ` sets the resolution and can be chosen automatically as the scale that resolves half
the data's entropy. Sorting the data and writing `ψ` as rising and falling exponentials
...
κ = kappa_interval(x).κ              # principled smoothing scale (half-entropy point)
